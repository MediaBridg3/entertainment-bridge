<template>
  <div class="connection-modal">
    <div class="connection-modal__modal">
      <div class="modal-header">
        <button @click="emitClose"><Icon name="cross-circle" /></button>
      </div>
      <h3 class="mb-4">Add connection</h3>

      <form @submit.prevent="initSearch" class="main-search__form">
        <div class="w-100">
          <input type="text" v-model="searchTerm" />
          <button><Icon name="search" /></button>
        </div>
        <div class="search-icons d-none d-md-flex">
      ...
      <div
        v-if="pending || media.length > 0"
        class="box connection-search-list mt-4"
      >
        <template v-if="pending">
          <Spinner />
        </template>
        <template v-if="media.length > 0">
          <ConnectionSearchList :data="media" />
        </template>
      </div>
    </div>
  </div>
...
import ConnectionSearchList from '~~/components/connections/ConnectionSearchList.vue';
      ...
      if (!store.twitchToken) {
        ...
        if (this.searchTerm && this.searchTerm.length > 0) {
          ...
          const data = await fetch(
            `/api/multi-search?search=${this.searchTerm}`
          );
          if (data) {
